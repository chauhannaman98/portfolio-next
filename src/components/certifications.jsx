    ...
    <section id="certifications" className="relative py-20 md:py-28">
      ...
      <div className="relative mx-auto max-w-6xl px-5 sm:px-8">
          ...
          {certifications.map((cert, i) => {
            ...
            return (
              <Reveal key={cert.title} delay={0.06 * i} className="h-full">
                <div className="card-lift group relative flex h-full flex-col overflow-hidden rounded-xl border border-white/[0.07] bg-white/3 p-6">
                  <div
        ...
        <Reveal className="mt-14 md:mt-16">
          ...
          <ul className="mt-6 divide-y divide-white/6 overflow-hidden rounded-xl border border-white/[0.07] bg-white/3">
            {achievements.map((item) => {
              const Icon = achievementIcons[item.icon] || Trophy;
              return (
                <li
                  key={item.title}
                  className="group flex items-start gap-4 p-5 transition-colors duration-300 hover:bg-white/3 sm:p-6"
                >
